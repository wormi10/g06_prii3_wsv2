    ...
    def image_callback(self, msg):
        ...
        if ids is not None and len(ids) > 0:
            ...
            for i, marker_id in enumerate(ids.flatten()):
                ...
                # Añadir texto con ID y posicion (TEXTO PEQUEÑO Y A LA IZQUIERDA)
                corner = corners[i][0]
                
                # Encontrar la esquina más a la izquierda (menor X)
                left_corner = min(corner, key=lambda p: p[0])
                
                # Texto más compacto
                pos_text = f"ID:{marker_id} ({tvec[0][0]:.2f},{tvec[0][1]:.2f},{tvec[0][2]:.2f})"
                size_text = f"{current_marker_size:.2f}m"
                
                # Posición del texto: a la izquierda del marcador
                text_x = int(left_corner[0]) - 10
                text_y = int(left_corner[1])
                
                # Dibujar fondo semi-transparente para mejor legibilidad
                (text_w, text_h), _ = cv2.getTextSize(pos_text, cv2.FONT_HERSHEY_SIMPLEX, 0.35, 1)
                ...
                cv2.putText(
                    ...
                    0.35,  # Tamaño de fuente más pequeño (antes 0.4)
                    (0, 255, 0),
                    1
                )
                
                # Texto secundario (tamaño) debajo
                cv2.putText(
                    cv_image,
                    size_text,
                    (text_x, text_y + 15),
